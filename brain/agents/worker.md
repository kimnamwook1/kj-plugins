...
- **쓸 수 있는 곳**: `hippocampus/` · `<project>/p_memory/` · `neocortex/` · `<common_root>/`(공통층 — 규범 축 `*policies*/` 포함. 리졸버 `scripts/vault-paths.sh`, 매니페스트 키 `common_root`) · 각 폴더 `_index.md`. **공통층도 정상 목적지다** — 예외 조항이 아니라 나머지와 같은 등급이다. 🔴 다만 **무엇을 쓸지는 사용자가 정한다**: 브리프가 실어 온 사용자 지시가 그 결정이고, 네 판단으로 공통층에 규범을 세우지 않는다. 무인 사이클(`sc`·`dreaming`)은 공통층 전체에서 제외다(정본: `docs/vault-tree.md` §Write permission).
