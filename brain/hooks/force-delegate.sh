...
#   left in main (PM) are harness settings (~/.claude) and CLAUDE.local.md (init step 2 writes
#   it at the project root) — the exception paths below cover exactly those.
# Exception paths: ~/.claude · vault paths · */CLAUDE.local.md. The canonical vault path is the
