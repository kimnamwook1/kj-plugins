...
- 🔴 **`unresolved` counts distinct targets, not occurrences, so the total under-reports the damage.** `verbose` emits one row per target — `<target>` · `<occurrence count>` · `<the files holding them>` — and `total` is the number of *rows*. A stem with two dozen inbound links therefore contributes exactly 1. Measured 2026-08-12 on vault `techtainment`, one run: `unresolved total` = **427** while the per-target occurrence counts in `verbose` sum to **2350**, a factor of 5.5, with a single target (`actor-tag-domain-model`) holding 27 of them across 20 files. That ratio is vault state and will move; **the counting basis is the durable fact.** Consequence: a change that breaks many links into one note is nearly invisible in the total, so anything checking a move or a rename diffs the **target list** and not the number (`docs/knowledge-convention.md` §Filename).
