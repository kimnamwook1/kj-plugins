...
> Tree axes (common layer · projects root · tools root) are **not** prerequisites and **not** literals in this file — `scripts/vault-paths.sh` resolves them from the vault's `.brain-paths` manifest, defaulting to the pre-restructure layout when the manifest is absent.
...
```bash
# Meta-file exclusion convention: `! -name 'index.md' ! -name '_index.md' ! -name '0.*'` — all three are required.
...
. "${CLAUDE_SKILL_DIR}/../../scripts/vault-paths.sh"         # → BRAIN_COMMON · BRAIN_TOOLS · brain_projects · brain_find_notes
...

{
  ...
  #      → name-based exclusion is the **only line of defense**. Do not delete the exclusion convention above (index.md/_index.md would surface straight into recall).
  ...
  [ -n "$BRAIN_TOOLS" ] && [ -d "$BRAIN_TOOLS" ] && CROOTS[${#CROOTS[@]}]="$BRAIN_TOOLS"   # manifest `tools_root` (vault-paths.sh); empty value or missing dir = not scanned
  if [ ${#CROOTS[@]} -gt 0 ]; then
