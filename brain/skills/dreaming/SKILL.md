...
- **reject-log recurrence scan**: comb `<project>/knowledge/0.rejected.md` for rejects that **recur across sessions (similar raw text ≥2 times)**. The primary gate (sum < 3) sees only one session and can underrate reusability — if something discarded keeps coming back, that is the reuse signal. **Propose such items as re-promotion candidates** (scribe turns them into proper knowledge notes). Don't leave them discarded. Mark handled lines in the log (e.g. prefix `~`) so the next dream doesn't re-propose them.
...
- **stub-scan** — pick out only *referenced* stubs. Grep documents with `status: stub`, then count inbound wikilinks (`[[...]]`) across the whole vault for each filename·title. **Inbound ≥1 = a stub someone is waiting on** → report as fill-priority. Stubs with 0 inbound block no one, so count only (low priority).
