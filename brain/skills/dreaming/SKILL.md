...
## Target vault
The vault-root defined in the project's `CLAUDE.local.md` (recorded by `/brain:init` onboarding — no hardcoding). With multiple business vaults, iterate over each. **Tree axes are per-vault too**: `<BRAIN_COMMON>` below comes from each vault's `.brain-paths` manifest via the resolver `${CLAUDE_SKILL_DIR}/../../scripts/vault-paths.sh` (manifest `common_root`, default `000_common`) — never hardcode the layout.
...
Read the last dream date from `<BRAIN_COMMON>/dream-log.md` (if absent, first run = everything is in scope — format canon: §7). Narrow the target to knowledge·Facts `updated:` since then, plus newly created sessions. This is the incremental baseline.
...
- **Folder TOC (`_index.md`) creation·refresh + consistency check**: re-ensure every folder has a TOC — create `_index.md` for new folders; refresh existing ones to the current contents (wikilinks to files·subfolders). A legacy `index.md` is the same TOC under its pre-flip spelling — refresh it in place, never create `_index.md` beside it (vault-tree.md). **Fixing dead wikilinks that point to nonexistent filenames is low-risk automatic.** **Also check that index wording doesn't contradict the actual structure** — e.g. sessions are per-`<uid>.md` files, so if an index describes them as folders, correct it (spec drift = real recall cost). scribe appends in real time, but Dreaming periodically catches omissions·drift. Exclude `.obsidian`.
...
- Record this dream in `<BRAIN_COMMON>/dream-log.md`: date, scan scope, proposal/application summary, **the §6 metrics table**. This becomes the next dream's incremental baseline.
