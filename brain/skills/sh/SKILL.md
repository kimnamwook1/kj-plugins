...
3. **Progress 엔트리 — PM 직접 Edit**, `## Progress` 맨 위 삽입(기존 엔트리 보존·최신이 맨 위):
   ...
   - `next` 하위 1줄만 = 재개 첫 행동 — **파킹 시 필수**. 이 줄이 §6 시작 프롬프트의 본문이 된다.
...
6. **다음 시작 프롬프트 — 화면 출력만**(볼트 쓰기 없음):
   - 소스 = §3의 `next` 1줄 + Goal + 미완 To-Do. 세션에 없는 사실을 프롬프트에 넣지 않는다.
   - 형식 = 복붙 가능한 fenced block 1개, 3줄 이내:
     ...
     남은 것: <미완 To-Do 최대 3건>
     ```
   - 첫 줄은 `sr` 고정 — 파킹은 재개로 돌아온다. `ss`를 권하지 않는다.
7. **보고**:
   > 세션 파킹: `<경로>` (status: parked)
   > 재개는 `sr` · 끝난 작업이면 `sh`가 아니라 `sc`.
