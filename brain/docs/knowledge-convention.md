...
- **Scope**: note paths in the recall block (`[K]`·`[C]`·`[X]` lines) — **except `000_common/policies/`**, the signature tier no agent writes, counters included (`knowledge-escalate-convention`; a policies note is simply not counted). `[M]` lines point at **session files, not notes** — never counted, never marked.
