...
`<common_root>/` is a **fact record kept current by measurement**, plus one normative sub-axis (`*policies*/` — §The common layer). The unattended cycle never writes there — the path is refused twice, once before the write and once before the commit (`skills/dreaming/SKILL.md` §Paths). The path is read from `.brain-paths`; matching a literal would leave a vault whose value is `personal` undefended.

🔴 **Row 2 is a normal path, not an exception to be argued for.** The common layer sits at the same grade as every other destination in this table: an AI acting on a user instruction writes it, `*policies*/` included. **What separates the two rows is the presence of a user instruction, not whether the writer is an AI** — so the split to hold onto is *decision* vs *keystrokes*. The user decides what becomes a norm; the AI does the writing, the same as anywhere else. Reading "a norm is a user action" as "an agent may not write the file" is the misreading, and it is not hypothetical: measured 2026-08-12, `013_kj-plugins/p_memory/0.rejected.md` carries **9 common-policy candidates returned across five sessions** (2026-07-27 → 2026-08-05), every one annotated `사용자 서명 대기 · 볼트 공통층 policies/ 미기록`. They had no reachable destination, so they accumulated in a log instead of the vault.
...
A restructured common layer is legal as-is, and it restructures back — the techtainment vault has been measured at both ends. **2026-07-30**: `common_root: org` with `about · machines · networks · org_policies · platforms`. **2026-08-12**: the same root with `about · machines · network · platforms · policies`, the normative folder holding `common_policy.md · ORG_COMPLIANCE.md · SECRET_MANAGEMENT.md · _index.md`. 🔴 **Both are legal and neither is the canon** — which is exactly why the rule above matches a *segment containing* `policies` instead of a folder name, and why nothing downstream may hardcode either spelling. On a contradiction the normative axis wins ([[project-docs-convention]] §Document Conflict Precedence), which is why **no agent decides a norm on its own** — the deciding is the user's, the writing is not (§Write permission).
