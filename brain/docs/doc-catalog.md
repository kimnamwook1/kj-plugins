...
>
> **Agent branch names are not this section's call** — the coder's `<PREFIX>-<number>-<title-slug>` is harness canon (`agents/coder.md` §First action) and applies in every repo the harness runs in. §Delivery records the *human* convention alongside it when one exists.
