...
| **DESIGN (디자인 시스템 스펙 — FRD급)** | `design` | design | `docs/tech-design/` | When the product has a UI. **Design-system spec — FRD-grade**: design tokens · component inventory · states · interaction rules. **SSOT rule → [[project-docs-convention]] §Value Axes, "UI pixels" row — the only original, not restated here** (body template → [[doc-templates]]) | situational |
