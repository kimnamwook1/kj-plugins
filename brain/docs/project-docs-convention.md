...
- **Promotion is not proportionate.** Measured on the real vault 2026-08-12: **42 unknown-key warnings across the docs layer, and 0 of them are a retired key.** They are live kind-specific fields (`related_ticket`·`screen_id`·`priority`·`feasibility`·`methodology`…). Promoting the warn turns a 0-true-positive rule into 42 findings, which is how a gate gets ignored. ⚠ **The total is a moving number and the ratio is the load-bearing half** — that same day `summary` and `supersedes` left the warn stream entirely when `validate.sh` learned them as recognised keys (KJP-adhoc), which is the *correct* fix for a live field that warns: teach the linter the key, never promote the channel.
- 🔴 **Retired-ness is scoped, so there is no single list to hold — and the scope that bites hardest is the *layer*.** The same string carries a different **severity** depending on which layer the file sits on, and porting one list down would overwrite that. `species` is the worked example, in both directions at once: it is on the wiki layer's retired list, where it is a **finding** (`scripts/validate.sh` §wiki retired keys, pinned by `validate-selftest.sh`), and it is retired in docs too as of 2026-08-12 — where it must stay a **warn**, because a docs file may be an import and the bullet above forbids the forced rewrite. Port the wiki list here and `title` and `species` both become docs findings, which is precisely the outcome the two bullets above rule out. The per-*kind* axis remains real — each kind has its own template, so one kind can retire a key another keeps — but it has **no live in-docs instance today**: the feature template's `version:`/`species:`, which used to be that instance, were retired 2026-08-12 (KJP-adhoc), so the two keys are now retired across every docs kind. A merged list erases both distinctions.

🔴 **What the paragraph above does NOT cover: frontmatter that fails to parse (KJP-56, 2026-08-12).** The warn-only decision is about *vocabulary* — which key names are legal. An unquoted wikilink (`related: [[a]]`) is a **syntax** defect: YAML reads `[[` as a nested sequence, so the value is silently not a link, and the multi-link spelling makes the whole block unparseable and renders it as body text. That is a finding here exactly as it is on the wiki layer, and it is not a promotion of the unknown-key warn — a file can produce both, on both channels. Canon for the correct spelling, and the measured counts behind the rule, live in one place: [[knowledge-convention]] §related. Measured on the real vault: **2 docs-layer files carry it**, so scoping the check to the wiki layer would have left both permanently invisible.

🔴 **Widened 2026-08-12 (KJP-97): the wikilink is one instance, and the general rule is the one to read.** The same syntax axis covers *any* scalar YAML cannot parse — a value opening with an indicator (`` ` `` `@` `*` `,` `%`, or a flow collection left unclosed) and an unquoted value carrying a colon+space. Measured after the wikilink sweep landed: **11 blocks vault-wide still failed to parse and not one of them was a wikilink**, which is what proved the rule had been written to a key instead of to the scalar. A docs block that fails to parse loses its `status:`, and a folder index its `next_id:`, exactly the way a wiki note loses its `summary:` — so the finding reaches this layer on the same reasoning, and 0 docs-layer files carry it today. Canon: [[knowledge-convention]] §Frontmatter must parse.
...
- **`frd` and `tdc` are retired as separate kinds (KJP-84, 2026-08-12).** The two documents merged into one file, and a kind is derived from a path — one path cannot yield two kinds without the matrix becoming undecidable. The merged file's kind is `feature`, which is also what its folder already calls it — and that is the whole reason the `species:` field that used to restate it was retired (KJP-adhoc, 2026-08-12). Nothing mechanical moved with them: `validate.sh` never matched a feature path (its only path arms are `docs/adr/` and `docs/develop/API_SPEC.md`), so `kind` here is documentation vocabulary and this row is its only home.
...
- 🔴 **`history` is live on ADR too — it is not a per-kind retirement (KJP-94, 2026-08-12).** The transcript's `adr-editor` card listed it among the ADR retired fields; that was an error and the card was corrected. It does **not** fall under the per-kind rule three sections up: **`history` records format alignment, never a revision of the decision.** Measured on the real vault — 6 of 7 ADRs carry it, 8 entries total, **0 of which revise a decision** (6 are the KJP-19 vocabulary/filename/backfill batch, 2 are birth entries whose `at` equals the file's `updated`). The immutability rule bans writing a *reversal* into the old file (`superseded_by`), and nothing here does that. The clinching case: `status` is a **required** key and KJP-19 migrated its vocabulary (`Proposed`→`draft`, `Accepted`→`approved`), so those files had to be edited — retiring the field that records the edit would delete the record, not the edit. With `session` banned above, `history.ticket` is the only channel by which an ADR can say which ticket migrated it.
...
**`FRD` and `TDC` are one document (KJP-84, 2026-08-12).** The design canon merged them into one path: `docs/develop/feature/<pp>_<slug>_0000N.md`. (It also carried a `species: Feature — 기능 명세 (FRD+TDC 통합)` field saying the same thing a third time; that field was retired 2026-08-12 — KJP-adhoc.) Section order carries the old split — **what** (§Why · §Scope · §Rules · §Acceptance) then **how** (§Design). The two never were separate audiences here; a reader who needs the rules also needs the design.
